<script>
    ...
    onMount(async () => {
        if( isSessionUserProfile ){
            const data = userVerificationService.getUserVerification( $session.accessToken );
            verification = data;
        }
    });
</script>
...
<div class="ProfileVerification">
    ...
    {:else if isSessionUserProfile}
        {#if verification && verification.state.toLowerCase() == "inprogress"}
            <div class="ProfileVerification-card ProfileVerification-card--inprogress">
                <span class="ProfileVerification-title">
                    La empresa se encuentra en proceso de verificación.<br/>
                    En las proximas horas revisa <b style="color:#5387cc;text-decoration:underline;">el correo que se encuentra en
                    el certificado de cámara y comercio</b> para finalizar el proceso.
                </span>
            </div>
        {:else if verification && verification.state.toLowerCase() == "none" }
            <div class="ProfileVerification-card ProfileVerification-card--none">
                <span class="ProfileVerification-title">
                    ...
                    <a id="profile-submit-certificate-link" class="ProfileVerification-submit-link">
