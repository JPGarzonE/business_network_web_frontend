...
<div class="ProfileDNA">
    {#if editableMode && isSessionUserProfile}
        <Modal on:click={toggleEditableMode}>
            <DNAForm on:click={toggleEditableMode} afterSubmit={reloadComponentData} />
        </Modal>
    {/if}

    <h3 class="ProfileDNA-headline">ADN de la compañia</h3>

    <HorizontalScrollList id="DNA-list" beginningItemsNumber={DNAElements ? DNAElements.length : 0}>
        ...
        {:else if DNAElements && DNAElements.length <= 0}
            <div class="ProfileDNA-empty-card">
                <img class="ProfileDNA-empty-card-image" src="/static/images/profile_icon.svg" alt="icon_default" />
            </div>
