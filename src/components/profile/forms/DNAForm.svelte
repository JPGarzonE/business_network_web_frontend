...
<script>
    ...
    const categories = ["Equipo", "Procesos", "Capacitaciones", "Logros", "Metodologías", "Certificaciones", "Otro"];
    ...
    function validateOtherCategory() {
        if( category === OptionOther ){
            ...
            }else if( otherCategory && otherCategory.length > 0 && otherCategory.length < 3 ) {
        ...
        try {
            if( isSessionUserProfile ) {
                ...
                if( DNAElement && DNAElement.id )
                    ...
                    dnaElement = await submitCreate( dataToSubmit );
    ...
    async function submitUpdate( dataToSubmit ) {
        ...
        if( newMediaFile ) {
            const DnaElementData = await dnaService.updateUserDnaelementWithImage(
                $session.username, DNAElement.id, newMediaFile, dataToSubmit, $session.accessToken );

            return DnaElementData;
        }else {
            const DnaElementData = await dnaService.updateUserDnaelement(
                $session.username, DNAElement.id, dataToSubmit, $session.accessToken);
...
<style>
    ...
    .DNAForm-name, .DNAForm-category, 
    .DNAForm-category--other, .DNAForm-description {
        width: 100%;
        border: none;
...
<div class="DNAForm ProfileForm">
    ...
    <div class="DNAForm-headline">
        <h3 class="DNAForm-title">{ DNAElement ? "Actualiza tu ADN" : "Crea tu identidad" }</h3>
        ...
        {:else}
            {#if !DNAElement}
                <p class="DNAForm-subtitle">
                    ¿Qué te hace único? <br />
                    Aquí puedes mostrarle al mundo tu equipo, procesos,
    ...
    <div class="DNAForm-preview">
        <div class="DNAForm-preview-card {newMediaFile || (media && media.path) ? "" : "DNAForm-preview-card--without-image"}" >
            <div class="DNAForm-preview-media" >
                <ImageUploadInput id="DNAForm"
    ...
    <form class="DNAForm-form ProfileForm-form">
        <div class="form-group">
            <input type="text" name="name" placeholder="Título" class="DNAForm-name"
        ...
        <div class="form-group">
            <div class="ProfileForm-group">
                ...
                {#if category === OptionOther}
                    <input type="text" name="other-category" placeholder="¿Cuál?" class="DNAForm-category--other"
                        bind:value={otherCategory} on:input={validateOtherCategory} />
                {/if}
            </div>

            {#if otherCategoryFeedback}
                <p class="form-control-feedback form-control-feedback--invalid">{otherCategoryFeedback}</p>
        ...
        <div class="form-group">
            <textarea name="description" placeholder="Historia - Descripción breve" rows="2" class="DNAForm-description"
        ...
        <button on:click|preventDefault={submit} class="DNAForm-button button button--principal">
            Aceptar y mostrar
        </button>
