...
<div class="FileUploadInput">
    <label for={inputName} class="FileUploadInput-label" 
        style="border:3px solid {fontColor}; background-color:{ value && validType ? "var(--principal-color)" : "none" }">
        ...
        <span class="FileUploadInput-message" style="color:{fontColor};">
            ...
            {:else}
                {valueName ? valueName : message}
            {/if}
        </span>
    </label>
    <input type="file" id={inputName} name={inputName} accept={acceptFiles.join(",")}
        style="display:none;" on:change={setInputValue} />

    {#if validType === false}
        <p class="FileUploadInput-helper">Solo se admite: {acceptFiles.join(", ")}</p>
    {/if}
</div>