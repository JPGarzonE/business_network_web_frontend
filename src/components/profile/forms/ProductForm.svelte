    ...
    try {
      if (isSessionUserProfile) {
        ...
        if (type.toLowerCase() === 'producto') {
          ...
          if (product) productElement = await updateService(dataToSubmit);
          else productElement = await createService(dataToSubmit);
        }
...
<div class="ProductForm ProfileForm">
  ...
  <div class="ProductForm-headline">
    <h3 class="ProductForm-title">
      {product ? 'Actualiza tu producto' : '¿Qué estas vendiendo?'}
    </h3>
    ...
    {:else if !product}
      <p class="ProductForm-subtitle">
        ¡Sube tu producto acá! <br /> Te recomendamos subir fotos de buena calidad
        para aumentar las probabilidades de venta
      </p>
    {/if}
  </div>
  ...
  <div class="ProductForm-preview">
    <div
      class="ProductForm-preview-card {newMediaFile || (media && media.path) ? '' : 'DNAForm-preview-card--without-image'}">
      <div class="ProductForm-preview-media" />

      <h4 class="ProductForm-preview-name">{name ? name : 'Nombre'}</h4>
      <p class="ProductForm-preview-category">
  ...
  <form class="ProductForm-form ProfileForm-form">
    <div class="form-group">
      <select name="type" bind:value={type} class="ProductForm-type">
        {#each Types as t}
          <option value={t} selected={type.toLowerCase() == t.toLowerCase()}>
            {t}
          </option>
        {/each}
      </select>
    ...
    <div class="form-group">
      <input
        ...
        class="ProductForm-category"
        bind:value={category}
        on:input={validateCategory} />
